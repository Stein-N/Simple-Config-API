...
import net.xstopho.simpleconfig.reader.BufferedCharReader;
import net.xstopho.simpleconfig.reader.MalformedTomlException;
...
public class TomlDeserializer {
    public static TomlTable readTable(BufferedReader reader) throws IOException {
    ...
    private TomlTable readFile(BufferedCharReader reader) throws IOException {
        while (true) {
            ...
            if (character == '[') this.readTableHeader(reader);
            else this.readKeyValuePair(reader);

            // Skip remainder of the line
            while (reader.peekChar() != -1) {
                if (reader.peekChar() == ' ') reader.skipChar();
                    ...
                    break;
                } else throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered unexpected character '" + (char)character + "'!");
            }
        }
    ...
    private void readTableHeader(BufferedCharReader reader) throws IOException {
        reader.skipChar();
        if (reader.peekChar() == '[') throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Table arrays are not yet supported!");
        ...
        while (true) {
            ...
            if (character == ']') {
                if (keyBuilder.isEmpty()) throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Invalid key '" + String.join(".", key) + "." + keyBuilder + "' in table header!");
                reader.skipChar();
                break;
            }
            ...
            if (character == '\'' || character == '"') {
                ...
                if(keyBuilder.isEmpty()) throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered unexpected character '" + (char)character + "' in table header!");
                else key.add(keyBuilder.toString());

                keyBuilder = new StringBuilder();
            } else if (Character.toString(character).matches("[A-Za-z0-9_-]"))
                keyBuilder.append((char)character);
            else throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered invalid key character '" + (char)character + "' in table header!");
        ...
        this.currentTable = key.toArray(String[]::new);
    }
    ...
    private void readKeyValuePair(BufferedCharReader reader) throws IOException{
        ...
        while (true) {
            ...
            if(character == '\'' || character == '"') {
                ...
                if (keyBuilder.isEmpty()) throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered unexpected character '" + (char)character + "' in key!");
                else key.add(keyBuilder.toString());

                keyBuilder = new StringBuilder();
            } else if (Character.toString(character).matches("[A-Za-z0-9_-]"))
                keyBuilder.append((char)character);
            else throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered invalid key character '" + (char)character + "'!");
        ...
        if (character == '\'' || character == '"') {
            ...
            this.putKeyValue(key.toArray(String[]::new), TomlPrimitive.of(value));
        } else if(character == '+' || character == '-' || Character.isDigit(character)) {
            Number number = this.readNumber(reader);

            if (number instanceof Integer) this.putKeyValue(key.toArray(String[]::new), TomlPrimitive.of((int)number));
            else this.putKeyValue(key.toArray(String[]::new), TomlPrimitive.of((double)number));

        } else if (reader.peekChars(4).equals("true") || reader.peekChars(5).equals("false")) {
            boolean value = reader.readChars(4).equals("true");

            if (!value) reader.skipChar();
            this.putKeyValue(key.toArray(String[]::new), TomlPrimitive.of(value));
        } else if (character == '[') throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Array values are not yet supported!");
        else if (character == '{') throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Inline tables are not yet supported!");
        else throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Encountered unexpected character '" + (char)character + "' whilst reading value!");
    }
    ...
    private int readEscapeSequence(BufferedCharReader reader) throws IOException {
        ...
        if (character == '"') return '"';
        if (character == '\\') return '\\';
        if (character == 'u') return this.readUnicodeChar(reader, 4);
        if (character == 'U') return this.readUnicodeChar(reader, 8);
    ...
    private int readUnicodeChar(BufferedCharReader reader, int length) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            else throw new MalformedTomlException("(Line " + reader.getLineIndex() + ":" + reader.getCharIndex() + ") Invalid character '" + (char)character + "' in unicode escape sequence!");
        }
    ...
    private void putKeyValue(String[] key, TomlElement value) throws MalformedTomlException {
        ...
        for(int i = 0; i < path.length - 1; i++) {
            ...
            if (member == null) {
                ...
                object = newObject;
            } else if(member.isTable()) object = member.getAsTable();
            else throw new MalformedTomlException("Conflicting values '" + String.join(".", path) + "' and '" + String.join(".", Arrays.copyOfRange(path, 0, i + 1)) + "'!");
        }

        // Get the comment and hint from the old element
        if(object.get(path[path.length - 1]) != null) throw new MalformedTomlException("Key defined twice '" + String.join(".", path) + "'!");
